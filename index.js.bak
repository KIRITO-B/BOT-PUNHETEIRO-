	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = ｀ TCHAU FDP KKKK @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerB: '❌ SÓ O DEUS 𝑲𝑰𝑹𝑰𝑻𝑶 𝑶𝑭 𝑺𝑨𝑫ッ PODE USAR KRAIO ❌',
					admin: '❌ SÓ ADM PODE USAR! MEMBRO COMUM🤬🤬 ❌',
					Badmin: '❌ BOTA EU COMO ADM PRIMEIRO PORRA KKK ❌'
				}
			}
			...
			switch(command) {
				...
				case 'info':
					...
					teks = `*🌹𝑨𝑺𝑼𝑵𝑨-𝑺𝑨𝑵🌹* : ${me.name}\n*wa.me/+5511991796330* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Contato de bloqueio total* : ${blocked.length}\n*O bot está ativo em* : ${kyun(uptime)}`
					buffer = await getBuffer(me.imgUrl)
					client.sendMessage(from, buffer, image, {caption: teks, contextInfo:{mentionedJid: [me.jid]}})
					break
				case 'blocklist':
					teks = 'Esta é a lista de bloqueados número :\n'
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*NOME* 𝑲𝑰𝑹𝑰𝑻𝑶 𝑶𝑭 𝑺𝑨𝑫ッ\n*WPP:* wa.me/+5511991796330\n*INSTA:* OFF'})
					break
				case 'gostosin':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://imgur.com/a/uUnf0mF`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'O MAIS GOSTOSO DA WEB E O 𝑲𝑰𝑹𝑰𝑻𝑶 𝑶𝑭 𝑺𝑨𝑫ッ👉🏼👈🏼💖'})
					break
				/*case 'memeindo':
					memein = await kagApi.memeindo()
