	...
	client.on('chat-update', async (mek) => {
		try {
			...
			switch(command) {
				...
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://imgur.com/a/SJQEOvk.jpg`)
				...
				case 'bomdia':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Bom dia, acordei no ódio ❤️'})
					break
				case 'boatarde':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/JaO3yoV.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'ja ta de tarde? '})
					break
				case 'boanoite':
					memein = await kagApi.memeindo()
